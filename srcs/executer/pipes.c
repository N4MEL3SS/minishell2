...
void	exec_builtin_hdoc(t_mini_shell *in, t_list *arg_list)
{
	...
	if (ft_lstsize(arg_list) == 1 && in->is_heredoc)
	{
...
void	child(t_mini_shell *in, t_list *aux_list, int index)
{
	check_redirs(in);
...
void	sub_pipex(t_mini_shell *in, t_list *aux_list, int index, int *flag)
{
	pid_t	pid;
...
void	kill_last_process(t_mini_shell *in, int flag)
{
	if (flag)
