...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/01 14:36:30 by ncallie           #+#    #+#             */
/*   Updated: 2022/03/01 14:42:54 by ncallie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_errors_pipes_aux(t_mini_shell *in)
{
	int		count;
	char	c;
...
int	check_error_pipes(t_mini_shell *in)
{
	...
	while (in->user_in[in->f.i])
	{
		...
		{
			if (char_sp(in->user_in[in->f.i]) == 0)
				flag = 0;
			in->f.i++;
		}
	}
